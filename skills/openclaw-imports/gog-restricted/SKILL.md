...
- `gog drive ls --max N --json` — list files in root
- `gog drive ls --parent <folderId> --max N --json` — list files inside a specific folder
- `gog drive search '<query>' --json` — search files
- `gog drive get <fileId> --json` — fetch file metadata
...
Prefer `--parent <folderId>` for folder navigation. Do not guess positional folder arguments for `gog drive ls`.
...
- For Drive, start with `gog drive ls --max 10 --json` or `gog drive search '<query>' --json`, then narrow using file IDs.
