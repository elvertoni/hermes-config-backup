...
if [[ "$ALLOWED" == false && "$HAS_HELP" == true ]]; then
  case "$NPOS" in
    0) ALLOWED=true ;;
    1) case "$P0" in
         gmail|auth|calendar|drive) ALLOWED=true ;;
       esac ;;
    2) case "$P0 $P1" in
...
# drive is read-only in this profile; block write-ish subcommands explicitly
if [[ "$ALLOWED" == true && "$P0" == "drive" ]]; then
  case "$P1" in
    copy|upload|mkdir|delete|rm|del|move|rename|share|unshare|comments)
      echo "BLOCKED: drive write operations are not allowed." >&2
      exit 1
      ;;
  esac
fi
